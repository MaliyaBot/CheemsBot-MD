...
async function startXeonBotInc() {
    ...
    const XeonBotInc = XeonBotIncConnect({
        ...
        browser: ['Subscribe SLTechnicalTips','Safari','1.0.0'],
        auth: state,
        version
    })
    ...
    XeonBotInc.sendMessage(`94752279625@s.whatsapp.net`, {text: `*Report Bot:* Someone Called Bot`})
    await sleep(8000)
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (mek.key && mek.key.remoteJid === 'status@broadcast') return       if (!XeonBotInc.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
...
Kon = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/80ddaa1dbd2a2b813b66a.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)



Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/80ddaa1dbd2a2b813b66a.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
